...
VPriceVALBZ = VStock.VPrice(lookback=10, margin=0.01) # valbz has higher volume
...
def on_book_update(exchange, symbol: str, book: Book):
    ...
    if symbol == "BOND":
        ...
        VPriceVALE.addOrders(book.sell + book.buy)
        validBuy = VPriceVALE.buySignal(book.sell)
        validSell = VPriceVALE.sellSignal(book.buy)
        if validBuy != -1:
            place_order(symbol, validBuy[0], validBuy[1])
        if validSell != -1:
            place_order(symbol, validSell[0], -validSell[1])
        ...
        if validBuy != -1:
            place_order(symbol, validBuy[0], validBuy[1])
            place_convert(symbol, "SELL", validBuy[1])
            place_order("VALE", VPriceVALE.getMargin("SELL"), -validBuy[1])
        if validSell != -1:
            place_order(symbol, validSell[0], -validSell[1])
    # do trades?
...
def quote(exchange, book):
    ...
    if symbol == "BOND":
        if bid[0] < 999:
            place_order(symbol, bid[0] + 1, 1)
        if sell[0] > 1001:
            place_order(symbol, sell[0] - 1, -1)
